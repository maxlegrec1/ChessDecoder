...
Usage:
    uv run python chessdecoder/rl/train.py --config chessdecoder/rl/config.yaml
"""
...
from chessdecoder.rl.rollout import generate_rollouts, export_model
from chessdecoder.rl.sequence import parse_rollout, collate_rollouts
from chessdecoder.rl.log_probs import compute_ref_log_probs, compute_current_log_probs, compute_policy_entropy
from chessdecoder.rl.rewards import CompositeReward
from chessdecoder.rl.grpo import compute_group_advantages, normalize_advantages_minibatch, grpo_loss
from chessdecoder.rl.metrics import GRPOMetrics
...
def train():
    ...
    try:
        for outer_step in range(start_step + 1, config.num_outer_steps + 1):
            ...
            # 2. Generate rollouts in-process. With Phase 0's pre-allocated
            # KV cache and the B200's 191 GB VRAM, model + ref_model + engine
            # can all stay GPU-resident; no model.cpu()/model.to() shuttle.
