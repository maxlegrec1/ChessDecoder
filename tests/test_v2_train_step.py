"""Phase C: one full V2 training step (CPU). Validates the joint forward/
loss wiring and that gradients flow back into the board encoder (the encoder
is a submodule trained only via decoder+transition loss — markdowns/11 §0)."""
import chess
import pandas as pd
import torch
...
from chessdecoder.models.v2.model_v2 import (
    ChessDecoderV2, N_SQUARE_CLASSES)
from chessdecoder.utils.training import soft_bucket_loss
...
def _batch(P_max=8):
    ...
    for ply, uci in enumerate(["e2e4", "e7e5", "g1f3", "b8c6"]):
        rows.append({"game_id": 1, "ply": ply, "fen": b.fen(),
                     ...
                     "played_q": 0.05 * (ply + 1), "played_d": 0.3})
        b.push_uci(uci)
...
def test_v2_training_step_and_encoder_gradient():
    ...
    seq, pos = assemble_decoder_inputs(latents, move_emb, batch["wl"],
                                       batch["d"], m.fourier_encoder)
    h = m.decoder(seq)
    ...
    vmask = (batch["wdl_valid"] & batch["ply_mask"]).reshape(-1)
    wl_l = soft_bucket_loss(m.wl_head(h[:, pos["move_pos"], :].reshape(-1, 32)),
                            batch["wl"].reshape(-1), m.wl_bucket_centers, vmask)
    d_l = soft_bucket_loss(m.d_head(h[:, pos["wl_pos"], :].reshape(-1, 32)),
                           batch["d"].reshape(-1), m.d_bucket_centers, vmask)
    ...
    total = 5.0 * policy_loss + trans_loss + wl_l + d_l
    assert torch.isfinite(total)
    total.backward()

    # gradient must reach the encoder (it is trained only through the
    # decoder's policy/value loss + the transition loss).
