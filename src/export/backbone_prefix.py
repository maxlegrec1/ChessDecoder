"""
Exportable prefix (block-bidirectional) backbone for ONNX/TRT.

Full forward pass with explicit attention mask.
...
class BackbonePrefix(nn.Module):
    """Prefix backbone with explicit attention mask."""
    ...
    def __init__(self, num_layers, num_heads, head_dim, embed_dim, d_ff,
        ...
        # RoPE cache: [max_seq_len, head_dim//2, 2]
    ...
    def _apply_rope(self, x, input_pos):
        ...
        rope_vals = self.rope_cache[input_pos]  # [B, S, HD//2, 2]
        rope_vals = rope_vals.unsqueeze(2)       # [B, S, 1, HD//2, 2]
    ...
    def forward(self, input_ids, input_pos, attention_mask,
        ...
        Args:
            input_ids:       [1, S] int64
            input_pos:       [1, S] int64
            attention_mask:  [1, 1, S, S] float32 (0 or -1e9)
...
def from_chess_decoder(model):
    """Create BackbonePrefix from a trained ChessDecoder, copying all weights."""
    ...
    backbone = BackbonePrefix(num_layers, num_heads, head_dim, embed_dim, d_ff,
